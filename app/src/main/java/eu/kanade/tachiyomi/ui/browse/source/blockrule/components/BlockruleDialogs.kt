...
fun BlockruleCreateDialog(
    ...
    val ruleError by remember {
        derivedStateOf {
            rule.isBlank() || (rule.isNotEmpty() && ruleAlreadyExists)
        }
    }
    val regexError by remember {
        derivedStateOf {
            when (type) {
        ...
        text = {
            Column {
                OutlinedTextField(
                    ...
                    label = { Text(text = stringResource(KMR.strings.block_rule_name)) },
                ...
                FilledTonalButton(
                    ...
                    Text(stringResource(KMR.strings.block_rule_type))
                ...
                OutlinedTextField(
                    ...
                    label = {
                        Text(text = stringResource(KMR.strings.block_rule_rule))
                    },
                    supportingText = {
                        val msgRes = when {
                            rule.isBlank() -> KMR.strings.block_rule_cannot_be_empty
                            ruleError -> KMR.strings.block_rule_already_exist
                            regexError -> KMR.strings.block_rule_invalid_regex
...
fun BlockruleDeleteDialog(
    ...
    AlertDialog(
        ...
        text = {
            Column {
                Text(text = stringResource(KMR.strings.block_rule_confirm_delete))
                Text(text = stringResource(KMR.strings.block_rule_name) + ": " + blockrule.name)
                Text(text = stringResource(KMR.strings.block_rule_type) + ": " + blockrule.type.toShowName())
                Text(text = stringResource(KMR.strings.block_rule_rule) + ": " + blockrule.rule)
            }
        },
    )
}
...
fun BlockruleSortAlphabeticallyDialog(
    ...
    AlertDialog(
        ...
        title = {
            Text(text = stringResource(KMR.strings.block_rule_sort))
        },
        text = {
            Text(text = stringResource(KMR.strings.block_rule_confirm_sort_alphabetically))
        },
    )
}
